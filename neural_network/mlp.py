...
class MLPClassifier(NN):
    ...
    def train(self, trainloader, devloader=None, log=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            log_message = 'epoch: {} | train loss.: {} | valid loss.: {} | train acc: {}% | valid acc: {}% | time: {}'.format(
                epoch,
                loss_storage['train'][-1],
                loss_storage['valid'][-1],
                acc_storage['train'][-1],
                acc_storage['valid'][-1],
                self._get_time(starting_time, time.time())
            )

            if log:
                log.write('|'.join(log_message))
                log.write('----------------------------------------------------------------\n')
    ...
    def _nll_loss(self, dataloader):
        ...
        loss = round(loss / len(dataloader.dataset), 5)
        return loss
