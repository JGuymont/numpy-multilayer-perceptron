...
class MLPClassifier(NN):
    ...
    def backward(self, X, Y, crazy_loop=False):
        ...
        if crazy_loop:
            for x, y in zip(X, Y):
                ...
                self.grad_W_xh += self._get_gradient_W_xh(x, crazy_loop)
            ...
            self.forward(X, train=True)
            self.grad_W_hy = self._get_gradient_W_hy(Y) / batch_size
            self.grad_b_hy = self._get_gradient_b_hy() 
            self.grad_W_xh = self._get_gradient_W_xh(X, crazy_loop) / batch_size
            self.grad_b_xh = self._get_gradient_b_xh()
    ...
    def _get_gradient_b_hy(self):
        ...
        NOTE: maybe its a sum, not mean
        """
        return np.mean(self.grad_oa, axis=1).reshape(self.grad_oa.shape[0], 1) 

    def _get_gradient_W_xh(self, x, crazy_loop):
        ...
        """
        if not crazy_loop:
            grad_hs = self.grad_oa.T.dot(self.W_hy)
            self.grad_ha = 0
            for i in range(x.shape[0]):
                grad_hs_ha = np.diag(self._relu_derivative(self.ha[:, i]).reshape(self.hidden_size, ))
                self.grad_ha += grad_hs.dot(grad_hs_ha)
            #self.grad_ha /= x.shape[0]
    ...
    def _get_gradient_b_xh(self):
        ...
        """
        return np.mean(self.grad_ha.T, axis=1).reshape(self.hidden_size, 1)
